...
if (topbar && topbarInner && topLinks) {
  ...
  style.textContent = `
    ...
    .hmatias-ticker-track{display:inline-flex;align-items:center;min-width:max-content;animation:hmatiasTicker 34s linear infinite}
    ...
    .nav-menu a.active{color:var(--blue);border-color:var(--blue)}
    .hmatias-backtop{position:fixed;left:16px;top:112px;z-index:55;width:36px;height:36px;border-radius:50%;display:grid;place-items:center;background:rgba(255,255,255,.94);border:1px solid var(--border);box-shadow:0 8px 22px rgba(6,45,86,.12);color:var(--blue);font-size:18px;font-weight:900;opacity:0;pointer-events:none;transform:translateY(-6px);transition:opacity .25s,transform .25s}
    .hmatias-backtop.show{opacity:1;pointer-events:auto;transform:translateY(0)}
    .hmatias-backtop:hover{transform:translateY(-2px);box-shadow:0 10px 24px rgba(6,45,86,.16)}
...
// Mantém o menu ativo de acordo com a secção que está realmente visível.
...
// Seta discreta no canto superior esquerdo para voltar ao início.
